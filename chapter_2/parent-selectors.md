...
Примером может послужить поддержка IE, когда с помощью JavaScript в тег `body` или `html` добавляются классы `.ie6`, `.ie7` и им подобные. Возожность использования обратного объединения позволяет писать более гибкий код, когда нужно изменить контекст применения селектора.
...
Не редко требуется производить операцию склеивания имён текущего и родительского селектора. Такая практика применяется при создании новых классов на основе старого. Кроме того, такая операция просто необходима тем людям, кто использует технологию BEM (и ей подобные) при написании стилей.
...
Немного академический пример, который не несёт в себе смысловой нагрузки, но все таки отображает суть происходящего:
...
Советую вам не шалить с многократным использованием и комбинированием селекторов. Да, это круто, но такой функционал заставляет разработчиков лишний раз думать над его результатом.

А вот с объединением и сшиванием селекторов, напротив, я настоятельно рекомендую работать и применять эту технику практически всегда. Конечно, речь идёт об уместном использовании, а не повсеместном.
